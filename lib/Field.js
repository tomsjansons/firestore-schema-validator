...
class Field {
   ...
   */
  constructor(_label) {
    if (!_label || typeof _label !== 'string') throw new StructureError('Field Label must be defined.')
  ...
  async validate(fieldData) {
    if (this._defaultValue !== undefined && fieldData === undefined)
      fieldData = typeof this._defaultValue === 'function' ? await this._defaultValue() : this._defaultValue

    if (this._isOptional && fieldData === undefined) return this._OptionalSymbol

    if (this._isNullable && (fieldData === null || fieldData === undefined)) return null

    if (fieldData === undefined || fieldData === null) throw new ValidationError(`${this._label} is required.`)

    fieldData = await this.validateField(fieldData)

    if (this._objectOf) fieldData = await this.validateObject(fieldData)

    if (this._arrayOf) fieldData = await this.validateArray(fieldData)
  ...
  async validateObject(fieldData) {
    ...
    await asyncForEach(Object.entries(this._objectOf), async ([key, field]) => {
      const filteredValue = await field.validate(fieldData[key])

      if (filteredValue !== field._OptionalSymbol) filteredData[key] = filteredValue
    })
  ...
  async validateArray(arrayData) {
    ...
    await asyncForEach(arrayData, async itemData => {
      const filteredValue = await this._arrayOf.validate(itemData)

      if (filteredValue !== this._arrayOf._OptionalSymbol) filteredData.push(filteredValue)
    })
   ...
   */
  _defineType() {
    if (this._isTypeDefined) throw new StructureError('Type has already been defined.')
