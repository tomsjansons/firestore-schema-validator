...
class ModelClass {
  ...
  constructor(_snapshot, _data) {
    ...
    if (!this.constructor._collectionPath) throw new Error(`${name} must have a static getter _collectionPath.`)
    ...
    if (!this.constructor._schema) throw new Error(`${name} must have a static getter _schema.`)
  ...
  get _createdAt() {
    ...
    if (!createTime) return moment().toISOString()

    return moment.unix(createTime.seconds).toISOString()
  }
  ...
  get _updatedAt() {
    ...
    return moment.unix(updateTime.seconds).toISOString()
  }
   ...
   */
  get _docRef() {
    return this._collectionRef.doc(this._id)
  }
  ...
  static prehook(path, cb) {
    ...
    if (!Array.isArray(this._prehooks[path])) this._prehooks[path] = []
  ...
  static posthook(path, cb) {
    ...
    if (!Array.isArray(this._posthooks[path])) this._posthooks[path] = []
  ...
  static async getById(id) {
    const snapshot = await this._collectionRef.doc(id).get()
  ...
  static async _validateWhereClause(key, value) {
    const fieldStrings = key.split('.')
    const field = fieldStrings.reduce((all, curr) => {
      if (!all[curr]) {
        throw new Error(`'${curr}' is not a valid field name`)
      }
      return all[curr]._objectOf ? all[curr]._objectOf : all[curr]
    }, this._schema._fields)
    return await field.validate(value)
  }

  static async validateWhereClauses(whereClauses) {
    for (const whereClause of whereClauses) {
      whereClause.value = await this._validateWhereClause(whereClause.key, whereClause.value)
    }
    return whereClauses
  }
  ...
  static async getBy(key, value) {
    value = await this._validateWhereClause(key, value)
  ...
  static getBySubscribe(key, value, subscriptionCallback) {
    const unSubPromise = this._validateWhereClause(key, value).then(checkedValue => {
      const query = this._collectionRef.where(key, '==', checkedValue)
  ...
  static async getAllBy(key, value, optionalModifiers) {
    value = await this._validateWhereClause(key, value)
    let query = this._collectionRef.where(key, '==', value)

    if (Array.isArray(optionalModifiers) && optionalModifiers.length > 0)
      for (const modifier of optionalModifiers) {
        modifier.value = await this._validateWhereClause(modifier.key, modifier.value)
        query = query.where(modifier.key, modifier.opStr, modifier.value)
      }

    const querySnapshot = await query.get()

    return querySnapshot.docs.map(snapshot => new this(snapshot))
  }
  ...
  static getAllBySubscribe(key, value, optionalModifiers, subscriptionCallback) {
    const unSubPromise = this._validateWhereClause(key, value).then(checkedValue => {
      let query = this._collectionRef.where(key, '==', checkedValue)
      ...
      if (Array.isArray(optionalModifiers) && optionalModifiers.length > 0) {
        for (const modifier of optionalModifiers) {
          validationPromises.push(
            this._validateWhereClause(modifier.key, modifier.value).then(validatedValue => {
              modifier.value = validatedValue
              query = query.where(modifier.key, modifier.opStr, modifier.value)
              return
            })
          )
  ...
  static async create(data = {}, id) {
    const snapshot = await (!id ? this._collectionRef.doc() : this._collectionRef.doc(id)).get()
    ...
    await instance._docRef.set(instance._data)
  ...
  async validate(data = {}, all = false) {
    if (all) return await this.constructor._schema.validate(data)
  ...
  async runHooks(hooks, data = {}) {
    ...
    for (const changedKey of this._changedKeys.keys())
      if (Array.isArray(hooks[changedKey])) await asyncForEach(hooks[changedKey], async cb => await cb(data, this))
